...
exports.findAll = (req, res) => {
    const priceT = req.query.priceT;
    const priceB = req.query.priceB;
    const instock = req.query.instock;
    const model = req.query.model;
    let condition = {};
    if(priceT) {
        condition.price = { [Op.lte] : +priceT };
    }
    if (priceB) {
        condition.price = { [Op.gte] : +priceB };
    }
    if(priceT && priceB) {
        condition.price = { [Op.gte] : +priceB, [Op.lte] : +priceT };
    }
    if (instock) {
        condition.instock = { [Op.like] : `%${1}%` }
    }
    if(model) {
